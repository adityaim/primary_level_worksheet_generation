...
# # Custom CSS for old-money aesthetic and fixes
# st.markdown("""
# <style>
...
#     /* Dark borders for all input fields */
...
#         border-radius: 50%;
#         animation: mesmerizing-spin 2s ease-in-out infinite;
#         margin-bottom: 2rem;
#         position: relative;
#     }
...
# if 'current_operations' not in st.session_state:
...
#     """Generate questions based on operation type and specifications"""
#     questions = []
...
#     else:
#         return [questions] * num_students

# # def create_pdf(questions_data, instructions, mode, num_students, operation_info):
# #     """Create PDF with worksheets in minimalist format"""
# #     buffer = io.BytesIO()
...
# #         for section_idx, (operation_questions, operation_name) in enumerate(zip(questions_data[0], operation_info)):
# #             story.append(Paragraph(f"{operation_name} Problems", section_style))
            
# #             for question_idx, question in enumerate(operation_questions):
# #                 formatted_question = question.replace(" = ", " = _____________")
# #                 story.append(Paragraph(f"{question_idx + 1}. {formatted_question}", question_style))
...
# #             for section_idx, (operation_questions, operation_name) in enumerate(zip(questions_data[student_idx], operation_info)):
# #                 story.append(Paragraph(f"{operation_name} Problems", section_style))
...
#                     formatted_question = question.replace(" = ", " = _____________")
#                 else:
#                     formatted_question = str(question)  # Convert to string if not already
#                 story.append(Paragraph(f"{question_idx + 1}. {formatted_question}", question_style))
...
#     """Display an enhanced mesmerizing loading spinner"""
#     beautiful_messages = [
...
#                 st.session_state.current_operations.append(f"{operation} ({num_questions} questions)")
...
#         if st.session_state.current_operations:
#             st.markdown("### Current Questions Added:")
#             for i, op in enumerate(st.session_state.current_operations, 1):
#                 st.write(f"{i}. {op}")
...
#         st.write(f"*Questions Added:* {', '.join(st.session_state.current_operations)}")
...
#                     pdf_questions = [st.session_state.questions]
#                 else:
#                     pdf_questions = []
#                     for student_idx in range(st.session_state.num_students):
#                         student_questions = []
#                         for question_set in st.session_state.questions:
#                             student_questions.append(question_set[student_idx])
...
#                     for key in ['step', 'questions', 'current_operations', 'operation_info', 'num_students', 'mode']:
